...
#[derive(Clone, Debug)]
pub struct UInt<const N: usize, F: Field>{
    bits: [Boolean<F>; N],
    value: BigUint,
}

impl<const N: usize, F: Field> UInt<N, F> {
    /// Construct a constant [`UInt`] from the native unsigned integer type.
    pub fn constant(value: impl Into<BigUint>) -> Self {
        let mut bits = [Boolean::FALSE; N];
        
        let mut tmp = value.into();
        for i in 0..N {
            bits[i] = Boolean::constant(tmp.is_one());
            tmp >>= 1;
        }
    ...
    pub fn from_bits_le(bits: &[Boolean<F>]) -> Self {
        ...
        let mut value = Some(0.into());
        for b in bits.iter().rev() {
            value.as_mut().map(|v| *v <<= 1);
            
            match *b {
                Boolean::Constant(b) => value.as_mut().map(|v| *v |= u8::from(b)),
                Boolean::Is(ref b) => match b.value() {
                    Ok(b) => value.as_mut().map(|v| *v |= u8::from(b)),
                    Err(_) => value = None,
                },
    ...
    pub fn rotate_right(&self, by: usize) -> Self {
        ...
        let new_bits = self.0.iter().skip(by).chain(&self.bits).take(N);
        
        for (res, new) in result.0.iter_mut().zip(new_bits) {
            *res = new.clone();
        }
        result.value = self
            .value
    ...
    pub fn xor(&self, other: &Self) -> Result<Self, SynthesisError> {
        ...
        let new_bits = self.0.iter().zip(&other.bits).map(|(a, b)| a.xor(b));
...
macro_rules! make_uint {
    ($name:ident, $native:ident, $mod_name:ident, $r1cs_doc_name:expr, $native_doc_name:expr, $num_bits_doc:expr) => {
        ...
        #[doc = " type."]
        pub mod $mod_name {
            impl<const N: usize, F: Field> R1CSVar<F> for UInt<N, F> {
                type Value = $native;
